...
export const useAuthStore = defineStore('auth-store', () => {
   ...
   * @returns
   */
  const loginByToken = async (backendToken: ApiAuth.Token) => {
    let successFlag = false

    const { token: _token, refreshToken } = backendToken
  ...
  const handleActionAfterLogin = async (backendToken: ApiAuth.Token) => {
