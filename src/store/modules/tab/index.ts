...
export const useTabStore = defineStore('tab-store', () => {
  ...
  const removeTab = (tab: Tab.MultiTab) => {
  ...
  const clearLeftTabs = (currentTab: Tab.MultiTab) => {
  ...
  const clearRightTabs = (currentTab: Tab.MultiTab) => {
    const currentIndex = tabs.value.findIndex(({ key }) => key === currentTab.key)
  ...
  const clearOtherTabs = (currentTab: Tab.MultiTab) => {
    const rootTab = getTabByRoute(routeStore.rootRoute)
  ...
  const activeTab = ref<Tab.MultiTab>()

  const setActiveTab = (tab: Tab.MultiTab) => {
    activeTab.value = tab
  }

  const setScrollPosition = (tab: Tab.MultiTab, scrollPosition: Tab.MultiTab['scrollPosition']) => {
